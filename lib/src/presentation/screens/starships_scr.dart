...
class StarshipsScr extends StatefulWidget {
  const StarshipsScr({super.key});

  @override
  State<StarshipsScr> createState() => _StarshipsScrState();
}

class _StarshipsScrState extends State<StarshipsScr> {
  final _scrollController = ScrollController();
  final _list = <String>[];

  List<StarshipEntity> starshipsList = [];
...
// Switch()
  ...
  void initState() {
    super.initState();
    context.read<StarwarsCubit>().getStarships(GetStarshipsParams(index: _currentPage));
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<StarwarsCubit, StarwarsState>(listener: (context, state) {
      ...
      if (state is GotStarships) {
        starshipsList.addAll(state.starships);
      }
    }, builder: (context, state) {
      return Scaffold(
          backgroundColor: Colors.black87,
          appBar: customAppBar(context: context, title: "STARSHIPS"),
          body: state is GettingStarships && starshipsList.isEmpty
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return GestureDetector(
                        onTap: () {
                          Navigator.pushNamed(context, AppRoutes.toPlanets());
                        },
                        child: PhotoCard(
                          imageUrl: starshipsList[index].image,
                          title: starshipsList[index].name,
                        ));
                  },
                ));
    });
